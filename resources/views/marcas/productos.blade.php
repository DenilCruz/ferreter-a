...
<div class="shop-layout">
    ...
    <aside class="shop-aside">
        ...
        {{-- ── MARCAS ── --}}
        <h3 style="margin: 20px 0 12px 0; font-size: 1rem; color: var(--text-main); padding-top: 16px; border-top: 1px solid var(--border);">
            Marcas
        </h3>
        <ul style="list-style: none; padding: 0; margin: 0;">
            @foreach($todasLasMarcas as $m)
            <li style="margin-bottom: 4px;">
                <a href="{{ route('marcas.productos', $m->id) }}"
                   style="color: {{ $m->id == $marca->id ? 'var(--primary)' : 'var(--text-main)' }};
                          font-weight: {{ $m->id == $marca->id ? '700' : '400' }};
                          text-decoration: none; font-size: 0.9rem; display: flex; align-items: center; gap: 6px;">
                    @if($m->id == $marca->id)
